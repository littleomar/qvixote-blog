...
```
#### 火焰图（flame chart）概览
![image](https://image.855233.xyz/i/2026-05-06/25233c0d05aa4effad5839f32b04eb26.png)
在火焰图中可以看出所有的JS执行、DOM解析、渲染、绘制等都是在火焰图的第一行**Task**基础之上运行的，这个**Task**就对应着**主线程**，也就是我们经常说的JS的核心特性**单线程模型**
#### 标记 Marks
...
#### DOC请求
![image](https://image.855233.xyz/i/2026-05-06/764c3734f2524728b1819b6fb65d51cd.png)
...
![image](https://image.855233.xyz/i/2026-05-06/62dd2db110234e9d8739b4c4122f7dd4.png)
#### 流式编译
![image](https://image.855233.xyz/i/2026-05-06/5e6e8bd1db8342259d495f4469717a62.png)
...
![image](https://image.855233.xyz/i/2026-05-06/19034585beb346eb96ba8ddd7f41e17a.png)
...
![image](https://image.855233.xyz/i/2026-05-06/2f1ceb67165d48a9a4aa8fed6a782c97.png)
#### ❌ 疑问
在`React`项目中，构建DOM树、CSSOM树和渲染相关任务为什么会运行在microtasks之上？这点违背了浏览器主线程的原理
![image](https://image.855233.xyz/i/2026-05-06/50d4cd31e9924b27b5ad7e8c73ad7d88.png)
#### JS执行
此处分析一下HTML文档中引用的`main.tsx` `module`的执行过程：
![image](https://image.855233.xyz/i/2026-05-06/98f72ce8918841f48b6dc76a8d0ae789.png)
...
![image](https://image.855233.xyz/i/2026-05-06/bfbec17315664455b4739a31f960c524.png)
...
![image](https://image.855233.xyz/i/2026-05-06/577bbf36b2cc488aab9873a20821d644.png)
![image](https://image.855233.xyz/i/2026-05-06/98a0a683a5d544abb5d903acf07ed34e.png)
...
![image](https://image.855233.xyz/i/2026-05-06/30f54d5bd9f04c7e97b21c3b3fee9e8a.png)在火焰图中更容易看出在JS执行结束后会运行`Run microtasks`，我们可以把微任务的执行看作属于此次JS执行(Evaluate module)的一部分，因此在微任务中所创建的微任务属于此次执行过程，这样宏任务和微任务的执行顺序就很明了了，而且每一个`Run microtasks`之前必定会有个JS执行过程。
**垃圾回收**：浏览器的垃圾回收也指的是[[JS中的垃圾回收]]，在火焰图中的表现如下
![image](https://image.855233.xyz/i/2026-05-06/861ab489831444c384d7600853995eb9.png)
**用户操作**浏览器在监听到事件后（例如‘点击’操作）会在主线程中开启一个`Event:xxx`的任务且这个执行过程是由`Function Call`和`Run microtasks`一同组成的，可以看作是一个宏任务
![image](https://image.855233.xyz/i/2026-05-06/1e5fd52a53824c31acca2ebd4f443c68.png)
